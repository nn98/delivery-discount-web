...
> 이 문서의 원본은 모노레포 `docs/ANALYTICS.md`다. 여기 사본은 이 저장소만
> 보는 사람을 위한 것이라, 아래 링크 중 이 저장소에 없는 것은 모노레포
> (`woowacourse-teams/2026-to-discount`)에서 찾는다.
...
```bash
cd <monorepo> && bash scripts/ab_report.sh   # 스크립트는 모노레포에 있다
```
...
- 모노레포 `api/docs/traffic-analytics.md` — 서버 수집·릴레이 계약
- [README.md](../README.md) — 프론트 계측과 개인정보 고지
- 모노레포 `api/docs/decisions/ADR-005-first-party-analytics.md` — 자체 수집을 둔 배경
